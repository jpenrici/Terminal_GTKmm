...
void Terminal::setup_signals() {
    m_btn_input_execute.signal_clicked().connect(sigc::mem_fun(*this, &Terminal::on_execute_command));
}
...
void Terminal::on_menu_file_quit()
{
    close();
}
...
void Terminal::create_menu()
{
    ...
    // Execute menu
    auto execute_menu = Gio::Menu::create();
    execute_menu->append("Execute", "app.run");
    menu_model->append_submenu("Tools", execute_menu);
    ...
    if (app) {
        ...
        app->add_action("saveas", sigc::mem_fun(*this, &Terminal::on_menu_file_saveAs));
        ...
        app->add_action("about", sigc::mem_fun(*this, &Terminal::on_menu_help_about));
    }
}
...
void Terminal::on_execute_command()
{
    ...
    try {
        ...
        auto output = execute_shell_command(command);
...
auto Terminal::execute_shell_command(const std::string& command) -> std::string {
    ...
    if (status != 0) {
        throw std::runtime_error("Command execution failed with status " + std::to_string(status));
    }
